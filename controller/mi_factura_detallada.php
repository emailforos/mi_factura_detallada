...
//      $pdf_doc->fdc_orden = $this->factura->numero2;
      ...
      if($this->impresion['print_dto'])
      {
         ...
         $pdf_doc->SetWidths(array(20, 80, 10, 20, 10, 20, 22));
      ...
      if($lineas)
      {
         ...
         for($i = 0; $i < count($lineas); $i++)
         {
            ...
            if ($ealb != utf8_decode($lineas[$i]->albaran_codigo())){
                //31.8 Abrimos alb y leemos el num.pedido cliente
   ...
   private function enviar_email($doc, $tipo = 'detallada')
   {
      if($this->empresa->can_send_mail())
      {
         ...
         if($doc == 'factura')
         {
            $filename = 'Factura ' . $this->factura->codigo . ' ' . $this->idioma->fix_html($this->factura->nombrecliente) .'.pdf';
