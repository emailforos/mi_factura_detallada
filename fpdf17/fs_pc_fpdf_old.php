...
{	
    ...
    function Header()
    {
        ...
        {
            $this->Image('tmp/'.FS_TMP_NAME.'logo.png',$this->fdf_Xlogotipo,$this->fdf_Ylogotipo,75);
            $this->Ln(0);
        }
	...
	$this->addProveedor($this->fdf_codproveedor);
        $this->addPageNumber($this->PageNo().'/{nb}');

        // Datos del Cliente
        $cliente  = $this->fdf_nombreproveedor . "\n";		
        ...
        for($i=0;$i<count($this->datoscab);$i++)
        {	
            $this->Rect($aquiX, $aquiY, $this->widths[$i], 155, 1, 'D');
            $aquiX += $this->widths[$i];
        }
    }
    ...
    function addProveedor ( $ref )
    {
        ...
        $y2  = $y1;
        $mid = $y1 + ($y2 / 2);
	  $texte  = 'C'.chr(243).'digo proveedor: ' . $ref;    
        $this->SetXY($w,32);
        $this->SetFont( "Arial", "", 9);
        $this->Cell(0,4,$texte,0,0,'R');
    }
    ...
    function addPago( $mode )
    {
        ...
        $y2  = $y1+10;
        $mid = $y1 + (($y2-$y1) / 2);
	  $texte  = 'Condiciones de pago: ' . $mode;    
        $this->SetXY(10,$h-41,5);
        $this->SetFont( "Arial","B",9);
